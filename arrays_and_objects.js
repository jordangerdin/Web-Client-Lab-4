...
let sep= '~~~~~~~~~~~~~~~~~~~~' // Seperator for readability.
...
console.log(euros * conversionRate)

maxRate = 0
currency = ''

for (rate in rates) {
    if(rates[rate] > maxRate){
        maxRate = rates[rate]
        currency = rate
    }
}

console.log(`The currency with the highest conversion rate from euros is ${currency}, at the rate ${maxRate}`)
...
let newCatOwner = {name: 'Taylor Swift', cat: 'Meredith'}
cats_and_owners.push(newCatOwner)

for (catownerpair in cats_and_owners){
    console.log("Owner: " + cats_and_owners[catownerpair].name + "\tCats name: " + cats_and_owners[catownerpair].cat)
}
